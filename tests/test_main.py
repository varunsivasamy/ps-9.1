...
propose_action() is mocked throughout, so these never touch the Anthropic API.
...
from autonomy_engine.agent_actions import AgentAction, AgentActionError
...
def _action(kind: str) -> AgentAction:
    ...
    if kind == "update":
        return AgentAction(
            ...
            description="Set phone_number on customer C-10482",
            tool_name="update_customer_record",
            parameters={"customer_id": "C-10482", "field": "phone_number"},
...
def _propose(client: TestClient, kind: str, session_id: str = SESSION) -> dict:
    with patch("autonomy_engine.main.propose_action", return_value=_action(kind)):
...
def test_propose_read_routes_autonomous_and_executes(client):
    ...
    assert body["risk_score"]["composite_score"] < 0.3
...
def test_propose_update_routes_confirm_with_preview(client):
    ...
    assert "phone_number" in body["preview"]
    assert 0.3 <= body["risk_score"]["composite_score"] <= 0.7
...
def test_propose_bulk_delete_routes_full_review(client):
    ...
    assert body["risk_score"]["composite_score"] > 0.7
...
def test_resolve_confirmation_confirm(client):
    ...
    assert response.status_code == 200
    assert response.json() == {"status": "confirmed", "reviewer": "alice@example.com"}
...
def test_resolve_review_approve(client):
    ...
    assert response.status_code == 200
    assert response.json() == {"status": "reviewed", "reviewer": "bob@example.com"}
